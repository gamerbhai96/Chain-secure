...
const readFraudSignalFlags = (analysis: AnalysisResponse): unknown[] => {
  ...
  const blockchain = (detailed as { blockchain_analysis?: unknown }).blockchain_analysis;
  if (!blockchain || typeof blockchain !== 'object') return [];

  const fraudSignals = (blockchain as { fraud_signals?: unknown }).fraud_signals;
  if (!fraudSignals || typeof fraudSignals !== 'object') return [];

  const detailedFlags = (fraudSignals as { detailed_flags?: unknown }).detailed_flags;
  return Array.isArray(detailedFlags) ? detailedFlags : [];
};
...
const REPORT_COLORS = {
  ...
  warning: [181, 139, 46],      // Warm amber #b58b2e
  danger: [166, 61, 61],        // Deep red #a63d3d
  muted: [102, 102, 102],       // Neutral gray #666666
  light: [247, 245, 242],       // Warm paper #f7f5f2
  white: [255, 255, 255],
...
/**
 * Add ChainSecure logo to the PDF document using the local PNG file
 */
const addChainSecureLogo = async (doc: jsPDF, x: number, y: number, width: number, height: number): Promise<void> => {
  try {
    ...
    // Add the local PNG logo with proper dimensions
    doc.addImage(dataUrl, 'PNG', x, y, width, height);
  } catch (error) {
    console.warn('Failed to load ChainSecure logo:', error);

    // Fallback: Draw editorial text-based logo
    doc.setFillColor(196, 93, 62); // Terracotta
    doc.setDrawColor(196, 93, 62);
    doc.rect(x, y, width, height, 'FD'); // No rounded corners

    // Add ChainSecure text
    doc.setTextColor(255, 255, 255);
    doc.setFontSize(12);
    doc.setFont('helvetica', 'bold');
    doc.text('ChainSecure', x + width / 2, y + height / 2 + 2, { align: 'center' });
  }
};

/**
 * Draw editorial header for ChainSecure reports
 */
const drawProfessionalHeader = async (doc: jsPDF, title: string, subtitle: string): Promise<number> => {
  const { margin, pageWidth } = LAYOUT;

  // Clean editorial header - dark ink background
  doc.setFillColor(26, 26, 26); // Ink color
  ...
  const logoHeight = 30;
  const logoX = margin;
  const logoY = (LAYOUT.headerHeight - logoHeight) / 2;

  await addChainSecureLogo(doc, logoX, logoY, logoWidth, logoHeight);

  const textStartX = logoX + logoWidth + 18;

  // Title - clean white text
  doc.setTextColor(247, 245, 242); // Paper color
  doc.setFontSize(16);
  doc.setFont('helvetica', 'bold');
...
/**
 * Draw editorial footer with ChainSecure branding
 */
const drawProfessionalFooter = (doc: jsPDF, pageNum: number, totalPages: number): void => {
  ...
  // ChainSecure branding
  doc.setFontSize(7);
  doc.setTextColor(196, 93, 62); // Terracotta
  doc.text('ChainSecure', margin, pageHeight - 5);

  // Disclaimer
  doc.setFontSize(6);
  doc.setTextColor(120, 120, 120);
  doc.text('Blockchain Analysis Report', pageWidth / 2, pageHeight - 5, { align: 'center' });
};
...
/**
 * Draw a section header with editorial styling
 */
const drawSectionHeader = (doc: jsPDF, title: string, y: number): number => {
  const { margin } = LAYOUT;

  // Check if we're too close to the top of the page
  const headerHeight = LAYOUT.headerHeight + LAYOUT.footerHeight;

  // If we're too close to the top, add spacing
  let adjustedY = y;
  ...
  doc.setFontSize(14);
  doc.setFont('helvetica', 'bold');
  doc.text(title.toUpperCase(), margin, adjustedY + 3);

  // Terracotta underline
  doc.setDrawColor(196, 93, 62); // Terracotta
  doc.setLineWidth(1.5);
  doc.line(margin, adjustedY + 7, margin + 40, adjustedY + 7);
 ...
 */
export const buildRiskFactorsList = (analysis: AnalysisResponse): string[] => {
  const directFactors = Array.isArray(analysis.risk_factors) ? analysis.risk_factors : [];
  const predictorFactors = Array.isArray(
    analysis.detailed_analysis?.ml_prediction?.risk_factors
  )
    ? analysis.detailed_analysis?.ml_prediction?.risk_factors
    : [];
  const fraudSignalFlags = readFraudSignalFlags(analysis);
...
export const downloadExcelReport = (analysis: AnalysisResponse, address: string) => {
  ...
  // Risk factors data
  const riskFactors = buildRiskFactorsList(analysis).filter(
    (factor: string) =>
      factor &&
      ...
      !factor.toLowerCase().includes('timeout fallback') &&
      !factor.toLowerCase().includes('address-based estimates')
  );
  ...
  // Download the file
  writeFile(wb, `ChainSecure_Security_Report_${address.substring(0, 8)}_${new Date().toISOString().slice(0, 10)}.xlsx`);
};
...
export const downloadPdfReport = async (analysis: AnalysisResponse, address: string) => {
  ...
  doc.save(`ChainSecure_Security_Analysis_${address.substring(0, 8)}_${new Date().toISOString().slice(0, 10)}.pdf`);
};
...
export const downloadPdfReportWithCharts = async (analysis: AnalysisResponse, address: string) => {
  ...
  try {
    ...
    console.log('Chart elements found:', {
      ...
      balanceChartContent: balanceChart?.innerHTML.length,
      inflowChartContent: inflowChart?.innerHTML.length
    });

    if (balanceChart || inflowChart) {
      // Add new page for charts
      doc.addPage();
      let chartY: number = LAYOUT.margin;

      // Capture Balance History Chart
      if (balanceChart) {
        chartY = drawSectionHeader(doc, 'Transaction Flow Analysis', chartY) as number;

        const canvas = await html2canvas(balanceChart as HTMLElement, { scale: 2, useCORS: true, backgroundColor: '#ffffff' });

        // Create a clean canvas to draw onto
        ...
        if (ctx) {
          ...
          const imgData = cleanCanvas.toDataURL('image/jpeg', 0.9);
          const imgWidth = 170;
          const imgHeight = (cleanCanvas.height * imgWidth) / cleanCanvas.width;

          doc.setDrawColor(220, 220, 220);
          doc.rect(LAYOUT.margin, chartY, imgWidth, imgHeight);
          doc.addImage(imgData, 'JPEG', LAYOUT.margin, chartY, imgWidth, imgHeight);
          console.log('✓ Balance chart re-rendered and added.');
          chartY += imgHeight + 10;
        }
      }
      ...
      if (chartY > 180) {
        doc.addPage();
        chartY = LAYOUT.margin;
      }

      // Capture Inflow/Outflow Chart
      if (inflowChart) {
        chartY = drawSectionHeader(doc, 'Inflow vs Outflow Analysis', chartY) as number;

        const canvas = await html2canvas(inflowChart as HTMLElement, { scale: 2, useCORS: true, backgroundColor: '#ffffff' });
        ...
        if (ctx) {
          ...
          doc.rect(LAYOUT.margin, chartY, imgWidth, imgHeight);
          doc.addImage(imgData, 'JPEG', LAYOUT.margin, chartY, imgWidth, imgHeight);
          console.log('✓ Inflow chart re-rendered and added.');
        }
      }

      console.log('✓✓✓ Both charts processed successfully');
    } else {
      console.warn('No charts found - skipping chart section');
    }
  } catch (error) {
  ...
  doc.save(`ChainSecure_Intelligence_Report_${address.substring(0, 8)}_${new Date().toISOString().slice(0, 10)}.pdf`);
};